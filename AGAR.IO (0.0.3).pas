...
begin
  ...
  while step > 0 do
  begin
    FillRect(W_+X_-CamX-(nowx+1)*4,H_+Y_-CamY-(nowy+1)*4,W_+X_-CamX+(nowx+1)*4,H_+Y_-CamY+(nowy+1)*4);
    if dir 
  end;}
...
begin
  ...
  begin
    ...
    Enemy[i].ddX += Random(-8,8) / 32;
    Enemy[i].ddY += Random(-8,8) / 32;
    Enemy[i].dX += Enemy[i].ddX;
    if ABS(Enemy[i].dX) + ABS(Enemy[i].dY) > 2 then
    begin
      Enemy[i].dX -= Enemy[i].ddX;
      Enemy[i].ddX := 0;
    end;
    Enemy[i].dY += Enemy[i].ddY;
    if ABS(Enemy[i].dX) + ABS(Enemy[i].dY) > 2 then
    begin
      Enemy[i].dY -= Enemy[i].ddY;
      Enemy[i].ddY := 0;
    ...
    begin
      if Len2D(Enemy[i].X,Enemy[i].Y,Eat[j].X,Eat[j].Y) < Enemy[i].Score div 4+4 then
      begin
        Enemy[i].Score += 2;
        Eat[j] := new Point(Random(-Radius+2,Radius-2),Random(-Radius+2,Radius-2));
      end else
      if Len2D(Enemy[i].X,Enemy[i].Y,Eat[j].X,Eat[j].Y) < Enemy[i].Score div 4+16 then
      begin
...
begin
  ...
  if i <> j then
  if Len2D(Enemy[i].X,Enemy[i].Y,Enemy[j].X,Enemy[j].Y) < Enemy[i].Score div 4 + Enemy[j].Score div 4 + 8 then
  begin
...
begin
  ...
  for i: integer := 0 to NE-1 do
  begin
    if Len2D(X,Y,Eat[i].X,Eat[i].Y) < Score div 4+4 then
    begin
      Score += 2;
      Eat[i] := new Point(Random(-Radius+2,Radius-2),Random(-Radius+2,Radius-2));
    end else
    if Len2D(X,Y,Eat[i].X,Eat[i].Y) < Score div 4+16 then
    begin
